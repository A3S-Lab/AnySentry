...
if (!response.ok || !payload?.source?.sourceId || !payload?.token) {
  throw new Error(`Observer Source provisioning failed with HTTP ${response.status}`);
}

const replacements = {
  ANYSENTRY_SOURCE_ID: payload.source.sourceId,
  ANYSENTRY_INGEST_TOKEN: payload.token,
...
console.log(`Provisioned protected Observer Source: ${payload.source.sourceId}`);
