...
for repo in AnySentry Observer Sentry; do
  git -C "$ROOT/$repo" diff --quiet || fail "$repo working tree is dirty before source preparation"
  git -C "$ROOT/$repo" diff --cached --quiet || fail "$repo index is dirty before source preparation"
done
...
for repo in AnySentry Observer Sentry; do
  git -C "$ROOT/$repo" diff --quiet || fail "$repo was modified by source preparation"
  git -C "$ROOT/$repo" diff --cached --quiet || fail "$repo index was modified by source preparation"
done

echo "PASS build interface and isolated current-HEAD source preparation"
