...
assert.match(clickhouse, /DASHBOARD_PERSISTED_READ_MAX_BUCKETS = 360/);
assert.match(clickhouse, /requestedBuckets > DASHBOARD_PERSISTED_READ_MAX_BUCKETS/);
assert.match(clickhouse, /buildRanges\.slice\(0, DASHBOARD_BUCKET_BUILD_MAX_CHUNKS\)/);
assert.match(clickhouse, /this\.scheduleDashboardBucketWarm\(buildRanges, size\)/);
assert.match(clickhouse, /if \(this\.dashboardSnapshotWarmInFlight/);
assert.match(clickhouse, /persisted dashboard bucket cache unavailable; returning bounded hot fallback/);
